...
dataset_dir=path_of_sft_eval_datas
now_time=$(command date +%m-%d-%H-%M --date="+8 hour")
echo "now time ${now_time}"

datasets=(C-Eval CMMLU MMLU CMB_Exam MedQA_USMLE Med-C-Eval Med-CMMLU Med-MMLU)
langs=(zh zh en zh zh zh zh en)
splits=(test test test test test test test test)
n_shots=(0)
length=${#datasets[@]}
model_length=${#models[@]}
...
for (( i=0; i<$length; i++ )); do
  ...
  do
    CUDA_VISIBLE_DEVICES=${gpu} python -u eval_common_sft.py \
