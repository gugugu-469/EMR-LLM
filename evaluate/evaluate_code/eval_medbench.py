...
def evaluate(
    ...
    if use_vllm:
        ...
        sampling_params = SamplingParams(temperature=0.8, top_p=0.8,repetition_penalty=1.1, max_tokens = split_max_len,stop_token_ids=eos_token_id,seed=seed)
    else:
        if use_type == 'fp16':
    ...
    if os.path.isdir(dataset_dir):
        ...
        for task in tasks:
            file_path = os.path.join(dataset_dir,task)
    ...
    for file in files:
        ...
        print('训练集合:{}'.format(train_file))
        train_samples = []
        # 检查train集合是否存在，
        if not os.path.exists(train_file):
            if n_shot != 0:
                raise ValueError()('使用n_shot:{} 但是训练集合不存在'.format(n_shot))
        else:
            # train用于n-shots
